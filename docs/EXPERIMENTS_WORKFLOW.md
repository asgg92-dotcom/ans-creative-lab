...
| **확인** | `npm run dev` → `/experiments`에서 링크 클릭하여 `/projects/my-experiment` 확인 |
...
- **experiments.ts**: 실험(미완료) 프로젝트 등록 → `/experiments` 인덱스에서 목록 확인
- **/experiments**: 실험 목록 페이지, 클릭 시 `/projects/[slug]`로 이동
...
3. `npm run dev` → 브라우저에서 `/experiments` 접속 → 링크로 바로 확인
